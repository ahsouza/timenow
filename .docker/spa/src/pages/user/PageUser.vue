    ...
    <span slot="menu-left">
      ...
      <div class="row valign-wrapper">  
        <grid-vue tamanho="1 l6 m12">
          <router-link :to="'/user/' + adminPage.id + '/' + $slug(nome, {lower: true})">
...
export default {
  ...
  created() {
    ...
    if(userSession) {
      ...
      this.$http.get(this.$url + `user/` + this.$route.params.id, {"headers": {"authorization": "Bearer " + this.$store.getters.getToken}})
        .then(res => {
